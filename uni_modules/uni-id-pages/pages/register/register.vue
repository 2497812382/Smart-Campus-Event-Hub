		...
		<uni-forms ref="form" :value="formData" :rules="rules" validate-trigger="submit" err-show-type="toast">
			<uni-forms-item name="username" required>
				<uni-easyinput :inputBorder="false" :focus="focusUsername" @blur="focusUsername = false"
					class="input-box" placeholder="Please enter the username" v-model="formData.username" trim="both" />
			</uni-forms-item>
			<uni-forms-item name="nickname">
				<uni-easyinput :inputBorder="false" :focus="focusNickname" @blur="focusNickname = false"
					class="input-box" placeholder="Please enter the user nickname" v-model="formData.nickname" trim="both" />
			</uni-forms-item>
			<uni-forms-item name="password" v-model="formData.password" required>
				<uni-easyinput :inputBorder="false" :focus="focusPassword" @blur="focusPassword = false"
					class="input-box" maxlength="20"
					:placeholder="'please enter' + (config.passwordStrength == 'weak'?'6':'8') + '-16-bit password'" type="password"
			...
			<button @click="navigateBack" class="register-back">return</button>
			<match-media :min-width="690">
				<view class="link-box">
					<text class="link" @click="registerByEmail">Email verification code registration</text>
					<text class="link" @click="toLogin">Already have an account? Log in here</text>
				</view>
			</match-media>
		</uni-forms>
	</view>
